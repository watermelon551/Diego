...
class RunApplicationService:
    ...
    async def get_run_detail(self, run_id: str) -> RunDetailResponse | None:
        ...
        compile_bundle_ready = bool(
            run.input.generation_mode.value == "scratch"
            and (
        ...
        if compile_bundle_ready:
            artifacts["compile_bundle"] = {
                ...
                "provider": "diego",
                "mode": run.input.generation_mode.value,
        ...
        compile_bundle = CompileBundleResult(
            ...
            mode=run.input.generation_mode.value if compile_bundle_ready else None,
        )
        compile_result = CompileResult(
            status=str(getattr(run, "compile_status", "not_requested") or "not_requested"),
            requested_provider=getattr(run, "compile_requested_provider", None),
            provider=run.compile_provider,
            bundle_ready=compile_bundle_ready,
            artifact_path=run.pptx_path,
            artifact_ready=pptx_ready,
